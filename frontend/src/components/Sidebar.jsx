...
import { Building2, ClipboardList, Gauge, Landmark, LogOut, MapPinned, Settings, Table2, Users } from 'lucide-react';
...
export default function Sidebar() {
  ...
  const item = (href, label, note, Icon) => (
    <Link href={href} className={navLinkClass(isActive(href))}>
      <span className={cn('grid h-8 w-8 place-items-center rounded-md', isActive(href) ? 'bg-[#fff4f3]' : 'bg-white/35')}>
        <Icon className={iconClass(isActive(href))} />
      </span>
      <span>
        <span className="block text-sm font-medium leading-5">{label}</span>
        <span className={cn('block text-xs', isActive(href) ? 'text-[#66768a]' : 'text-[#4e6174]')}>{note}</span>
      </span>
    </Link>
  );
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        ...
        {user?.role === 'municipality_head' && (
          <section className="mb-6">
            <p className="px-3 pb-2 text-xs font-medium uppercase tracking-wide text-[#4e6174]">Municipality</p>
      ...
      {user && (
        <div className="border-t border-[#aebdcb] p-4">
          <div className="flex items-center gap-3 rounded-lg bg-white/45 p-3 ring-1 ring-[#aebdcb]">
            <div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-[#17212b] text-sm font-semibold text-white">{initials(user.name)}</div>
