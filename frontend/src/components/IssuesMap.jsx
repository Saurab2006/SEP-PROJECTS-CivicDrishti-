...
function getMapStatus(report) {
  if (report.status === 'duplicate') return 'merged';
...
function statusLabel(status, report) {
  if (status === 'duplicate') return report?.duplicateOf?.title ? `merged into "${report.duplicateOf.title}"` : 'merged - duplicate report';
          ...
          <div className="min-w-0 flex-1">
            ...
            {activeReport.status === 'duplicate' && (
              <p className="mt-1 text-xs font-semibold text-violet-700">
                Merged{activeReport.duplicateOf?.title ? ` into "${activeReport.duplicateOf.title}"` : ''}
              </p>
            )}
            {activeReport.status !== 'duplicate' && Number(activeReport.confirmations || 1) > 1 && <p className="mt-1 text-xs font-semibold text-violet-700">{activeReport.confirmations} citizen reports merged into one issue</p>}
