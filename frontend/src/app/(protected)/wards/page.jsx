...
export default function WardsPage() {
  ...
  return (
    <div className="mx-auto max-w-[1100px] space-y-5">
      ...
      {loading ? (
        <div className="grid grid-cols-2 gap-2 sm:gap-3 xl:grid-cols-3">{Array.from({ length: 6 }).map((_, i) => <div key={i} className="shimmer h-20 rounded-lg sm:h-24" />)}</div>
      ) : filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-3 xl:grid-cols-3">
          {filtered.map(w => (
            <Link key={w._id} href={`/wards/${w._id}`} className="flex items-center justify-between gap-1.5 rounded-lg border border-[#ded6c8] bg-white p-2.5 shadow-sm hover:border-[#0f3d3e] hover:shadow-md transition sm:gap-2 sm:p-4">
              <div className="min-w-0">
                <p className="text-xs font-black text-[#102a2b] flex items-center gap-1 sm:text-sm sm:gap-1.5"><MapPin className="h-3 w-3 shrink-0 text-[#dc143c] sm:h-3.5 sm:w-3.5" />Ward {w.ward}</p>
                <p className="mt-1 truncate text-[10px] text-[#65706c] sm:text-[11px]">
                  {[[w.municipality || w.district, w.district && w.municipality ? w.district : null].filter(Boolean).join(', '), w.province].filter(Boolean).join(' · ')}
                </p>
              </div>
              <ChevronRight className="h-3.5 w-3.5 shrink-0 text-[#cfc4b4] sm:h-4 sm:w-4" />
            </Link>
          ))}
        </div>
      )}
